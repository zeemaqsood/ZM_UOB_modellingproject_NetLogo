globals [element-scheme color-scheme color-scheme2 effective-receptors-link cluster-count
  dimer-count trimer-count tetramer-count pentamer-count hexamer-count septamer-count octamer-count enneamer-count decamer-count hendecamer-count dodecamer-count triskaidecamer-count tetrakaidecamer-count pentakaidecamer-count hexakaidecamer-count heptakaidecamer-count octakaidecamer-count enneakaidecamer-count icosamer-count plus-count
  count-group-list counter-list count-group-list-v2 counter-list-v2 count-group-list-v3 counter-list-v3 count-group-list-v4 counter-list-v4
  molecule-with-syks-list counter-molecule-with-syk-list
  molecule-with-receptors-list counter-molecule-with-receptor-list counter-receptor-list
  size-of-cluster  total-size mean-size r-absolute s-absolute l-absolute ll-absolute max-absolute
...
turtles-own [element-state product-id product-turn product-speed molecule cluster-id]
ligands-own [receptor-bound ligand-bound dimer-bound is-dimer? reversible?  l1 l2 ]
;dimers-own [family product-id product-turn site1-bounded?  site2-bounded?]
receptors-own [phosphorylated? ligand-bound protein-bound receptor-bound ]
syks-own [kinectically-active? inter-complex? site1-bound  site2-bound ]
...
to setup-globals
  ...
  ;set color-scheme [ 29 45 29 29 45 135 lime 135 lime lime lime black black red red white white ]
  set color-scheme [ sky black blue blue 65 135 lime 135 lime lime lime black black red red white white ]
  ;set color-scheme [ 28 45 28 28 45 blue lime blue lime lime lime black black red red white white ]
  ;set color-scheme [ 137 45 137 137 45 blue lime blue lime lime lime black black red red white white ]
  set effective-receptors-link limit-rr-links
  if limit-rr-links = "unlimited"
  [ set effective-receptors-link R ]
  set-patch-size my-patch-size
  resize-world world-dimension2 world-dimension world-dimension2 world-dimension
  ; for steady state
  ...
  set max-absolute (count patches / (r-size ^ 2))
  set tot-r-dict table:make

  set counter-molecule-with-receptor-list []
  set counter-receptor-list []
end
...
to setup-monomers
  set l-absolute ((L / 100) * (count patches / (r-size ^ 2)))
  create-ligands ( (L / 100) * (count patches / (r-size ^ 2)))
  [
    set shape "monomer"
    set size r-size * 0.8
...
; to set up receptors
to setup-receptors
  ;set r-absolute ((R / 100) * (count patches / (r-size ^ 2)))
  set r-absolute R
  ;create-receptors ( (R / 100) * (count patches / (r-size ^ 2)))
  create-receptors R
  [
...
; to setup syks
to setup-syks
  ;set S-absolute ((S / 100) * (count patches / (r-size ^ 2)))
  set S-absolute S
  ;create-syks ( (S / 100) * (count patches / (r-size ^ 2)))
  create-syks S
  [
    set shape "syk"
    set size r-size * 1.6
...
to setup-dimers
  ...
  ;set ll-absolute ((LL / 100) * (count patches / (r-size ^ 2)))
  ;create-ligands round(2 * LL / 100 * max-absolute)
  create-ligands round(2  * LL)
  [
    set shape "monomer"
    set size r-size * 0.75
  ...
  let i 0
  ;while [ i < round (2 * LL / 100 * max-absolute ) ]
  while [ i < (2 * LL) ]
  [
    let major item i dimeric-ligands
    let minor item (i + 1) dimeric-ligands
    ask major [ set l1 self set l2 [self] of minor  set ligand-bound lput ([self] of minor) ligand-bound create-link-with minor [ set hidden? true] ]
    ask minor [ set product-id [product-id] of major set l1 [self] of major set l2 self set ligand-bound lput ([self] of major) ligand-bound]
    set i (i + 2)
  ]
...
; randomly located.
to  setup-dense-region
  ask patches [ set pcolor 8 ]
  if dense-region?
  [

    let cnt-patches (dense-area / 100) * count patches  ; number of patches to be colored
    let yellow-space-num cnt-patches / dense-size              ; number of different regions to be formed which are of size 50
...
; go functions
; these functions implement dissociation, association and transformation
...
to go
  ...
  count-molecule-with-receptors
  count-molecule-with-receptors-for-hist
  ;histogram counter-molecule-with-receptor-list

;  ask links
;  [
;    if is-receptor? end1 and is-receptor? end2
;    [
;      show (word "END1:" [receptor-bound] of end1)
;      show (word "END2:" [receptor-bound] of end2)
;    ]
;  ]

  ;set-cluster-number
  ;show set-r-s-readouts
  ;set-r-s-readouts
  ;get-count-of-groups
  ;get-count-of-groups-v2
  ;get-count-with-double-bound-s
  ;write-into-file
  tick
end
...
to move
  ...
  [
    ...
    set product-speed (1 / size-of-product)
    ; ifelse size-of-product > 20 [ set product-speed (0.0001 / size-of-product) ]    [ set product-speed (0.1 / size-of-product) ]       ; reducing the speed based on the size of the product formed
    if pcolor = yellow
    [
      set product-speed product-speed * coefficient-of-diffusion ; in case of rafts initialised, reduce the speed in rafts
    ]

    if membrane-bound = true
    [
      ;ask monomers [ set product-speed product-speed * 0.2 ]
    ]
...
to asssociate-and-transform
  ...
  syk-activation
  if theme-4 [ ;show "before here 3"
    attaching-syk-within
    ;show "after here 3"
  ]
  if theme-6 [dimerize-ligands]
  if theme-8 [ligand-and-receptor]
  if theme-9 [;show "before here 4"
    syk-and-phosphorylated-r
    ;show "after here 4"
  ]
...
to dimerization-of-r
  ...
  [
    ...
    [
      ...
      [
        create-links-with other receptors with [product-id = [product-id] of myself and (not in-link-neighbor? [self] of myself) and member? self [receptor-bound] of myself]
        [set hidden? true ]
      ]
...
to attach-syk
  let the-molecules receptors-here with [phosphorylated? and protein-bound = nobody ]
...
to dimerize-ligands
  ...
  [
    ...
    [
      ...
      ask near-by-molecule
      [
        create-links-with other ligands with [product-id = [product-id] of myself and (not in-link-neighbor? [self] of myself) and member? self [ligand-bound] of myself]
        ; create-links-with other ligands with [product-id = [product-id] of myself ]
        [ set hidden? true ]
      ]
      update-molecule
    ]
...
to dissociate-and-reverse
  ...
  [
    upid ->
    ;show "START UPID"
    ;print word "UPID: " upid
    ask one-of turtles with [ product-id = upid ]
    [
      let element self
      if is-receptor? element
      [
;        show "before here receptor"
;        show self
;        print ( [molecule] of self )
;        print ( [element-state] of self )
         reverse-logic-for-receptors
;        show "after here receptor"
;        print ( [molecule] of self )
;        print ( [element-state] of self )
      ]
      if is-syk? element
      [
;        show "before here syk"
;        show self
;        print ( [molecule] of self )
;        print ( [element-state] of self )
...
;        print ( [element-state] of self )
      ...
      [
        ;show "before here ligand"
        ;reverse-logic-for-ligands
        ;show "after here ligand"
      ]
    ]
    ;show "END UPID"
...
;;in this function, myself = R under consideration for dissociation
  let processed? false
  ...
  [
    ...
    [
      ...
      [
        ...
        ; show connected-syk
        ask link-with connected-syk [ die ]
        set protein-bound nobody
        set element-state get-receptor-element-state
        set color get-color
        let current-product-id product-id
        let new-product-id product-id
        ask connected-syk
        [
        ...
        ifelse current-product-id = self
        [
          if any? other turtles with [nw:distance-to me = false and product-id = current-product-id]
          [
            ifelse any? receptors with [nw:distance-to me = false and product-id = current-product-id]
            [ set new-product-id ([self] of one-of receptors with [nw:distance-to me = false and product-id = current-product-id])]
            [ set new-product-id ([self] of one-of turtles with [nw:distance-to me = false and product-id = current-product-id])]

            ask other turtles with [nw:distance-to me = false and product-id = current-product-id]
            [
              set product-id new-product-id
              ;set molecule get-molecule-name product-id
            ]
            ask other turtles with [nw:distance-to me = false and product-id = new-product-id]
            [
              set molecule get-molecule-name product-id
            ]
          ]
    ...
    [
      ...
      [
        ask link-with connected-ligand [ die ]
        set ligand-bound nobody
        set element-state get-receptor-element-state
        let current-product-id product-id
        let new-product-id product-id
    ...
    [
      ...
      ;if ( protein-bound = nobody or [site1-bound] of protein-bound != one-of receptor-bound or [site2-bound] of protein-bound != one-of receptor-bound ) ;; making sure that the splitting R is not connected to the same Syk or LL, if Syk or LL are present
      ;;if protein-bound = nobody and ligand-bound = nobody and dissociate-receptor-and-receptor? ;; checking to see whether any ligand or syk is attached to this splitting R
      if dissociate-receptor-and-receptor?
      [
        ;show "before here 4"
        ;show self
        ;show connected-receptors
        ;show me
        ;show "before here 4b"
        let current-product-id product-id
        let new-product-id current-product-id
        set receptor-bound [] ;; un-dimerise myself
        __set-line-thickness -4
        set color get-color

        foreach connected-receptors ;; make the R-R link die
        [
          c-r ->
          ask link-with c-r [ die ]
          ask c-r
          [
            ifelse  member? me receptor-bound ; if this particular connected receptor is still part of the list of connected receptors of self
            [
              set receptor-bound remove me receptor-bound ;;remove this c-r from the list
              if empty? receptor-bound
              [
                __set-line-thickness -4 ;;when c-r's list is completely empty, reset appearance of outline border
                                        ;set color get-color
              ]
            ]
            [ show "not member"]
        ...
        set processed? true
        set color get-color ;;change from yellow dimers to back to the color palette
        ;show "after here 4b"
      ]
    ]
    ...
    ;show "after here  THE END of reverse-R"
  ]
...
to  unphosphorylation-process
  ...
  let theme-list  item 0 theme-rates
  let rate-list  item 1 theme-rates
  set rate-list (update-rate-list rate-list)   ; to reduce the rate of dephosphorylation within clusters according to size of cluster
  let transform-list? will-transform? rate-list
  if member? "theme-1" theme-list
  [
    ;show "before here 8"
...
to-report dissociate-receptor-and-syk? [xsyk]
  ...
  [
    if random-float 100 < reverse-rate-for-theme-4 and theme-4
    [report true ]
  ]
  ...
  [
    if random-float 100 < reverse-rate-for-theme-9 and theme-9
    [ report true]
  ]
  report false
end
...
to-report dissociate-receptor-and-ligand?
  if random-float 100 < reverse-rate-for-theme-8 and theme-8
  [ report true]
  report false
end

to-report dissociate-receptor-and-receptor?
  if random-float 100 < reverse-rate-for-theme-2 and theme-2
  [ report true]
  report false
end
...
to reverse-logic-for-syks
  ...
  [
    ...
    [
      ifelse site1-bound != nobody and  site2-bound != nobody and dissociate-one-site?
      [
        ...
        let current-product-id product-id
        let new-product-id current-product-id
        let lead [self] of connected-receptor
        ;show "before here 2SA TEST"
        if random 100 < 50
        [ set connected-receptor  site2-bound]
        ;show "before here 2S"
        ;show connected-receptor
        ask link-with connected-receptor [ die ]
        ;show "after here 2S"
        ...
        ifelse current-product-id = lead
        [
          if any? other turtles with [nw:distance-to lead = false and product-id = current-product-id]
          [
            ifelse any? receptors with [nw:distance-to lead = false and product-id = current-product-id]
            [ set new-product-id ([self] of one-of receptors with [nw:distance-to lead = false and product-id = current-product-id])]
            [ set new-product-id ([self] of one-of turtles with [nw:distance-to lead = false and product-id = current-product-id])]

            ask other turtles with [nw:distance-to lead = false and product-id = current-product-id]
            [
              set product-id new-product-id
            ]
            if nw:distance-to lead = false
            [
              set product-id new-product-id
            ]
            ask other turtles with [nw:distance-to lead = false and product-id = new-product-id]
            [
              set molecule get-molecule-name product-id
            ]
      ...
      [
        ...
        [
          ...
          ask connected-receptor
          [
            ;show "before here 8S inside the loop"
            set protein-bound nobody
            ...
            set color get-color
            update-molecule
            ;show "after here 8S inside the loop"
          ];show "after here 8S"
          update-molecule
        ]
        ;show "after here 1SB"
  ...
  ;show "after here 1S THE END"
end

to-report make-syk-inactive?
  if random-float 100 < reverse-rate-for-theme-3 and theme-3
  [
    report true
  ]
  report false
  ...
  [
    ...
    [
      ...
      [
        ...
        if [receptor-bound] of l1 = connected-receptor
        [ ask l1 [ ask link-with connected-receptor [ die ] ] ]
        if [receptor-bound] of l2 = connected-receptor
        [ ask l2 [ ask link-with connected-receptor [ die ] ] ]
        ask connected-receptor
        [
          set ligand-bound nobody
        ...
        [
          ...
          [
            ...
            ask l2 [ set product-id receptor-bound]
          ]
          [
            ask l2
            [ set receptor-bound nobody
            ...
            ask l1 [ set product-id receptor-bound]
          ]
          set element-state get-dimer-element-state
          set color get-color
        ]
      ...
      [
        ...
        ask l2
        [
          set product-id [product-id] of l1
          set element-state get-dimer-element-state
          set color get-color
        ]
        ask connected-receptor
...
to-report  dissociate-ligands?
  if random-float 100 < reverse-rate-for-theme-6 and theme-6
  [
    report true
  ]
  report false
...
to set-cluster-id
  ...
  if any? turtles with [count receptors with [product-id = [product-id] of myself] <= 2]
  [
    ask turtles with [count receptors with [product-id = [product-id] of myself] <= 2]
    [ set cluster-id nobody ]
  ]

  if any? turtles with [count receptors with [product-id = [product-id] of myself] > 2 and product-id = self]
  [
    ask turtles with [count receptors with [product-id = [product-id] of myself] > 2 and product-id = self]
    [
      ifelse cluster-id != nobody
      [
        ask turtles with [product-id = [product-id] of myself]
      ...
      [
        ifelse any? receptors with [ cluster-id != nobody and product-id = [product-id] of myself ]
        [
          let existing-cluster one-of receptors with [ cluster-id != nobody and product-id = [product-id] of myself ]
          set cluster-id [cluster-id] of existing-cluster
        ]
        [
          set cluster-id [who] of product-id
        ]
        ask turtles with [product-id = [product-id] of myself]
        [
          set cluster-id [cluster-id] of myself
...
to get-count-with-double-bound-s
  set count-group-list-v3 []
  set counter-list-v3 []
  let unique-products remove-duplicates [product-id] of syks with [ site1-bound != nobody and  site2-bound != nobody]
  foreach unique-products
  [ pid ->
    let s-count count syks with [ product-id = pid and site1-bound != nobody and  site2-bound != nobody]
    let combo (word s-count "S")
    if not member? combo count-group-list-v3
    [ set count-group-list-v3  lput combo count-group-list-v3
      set counter-list-v3  lput 0 counter-list-v3
    ]
    let index position combo count-group-list-v3
    let new-count ( item index counter-list-v3 + 1)
    set counter-list-v3 replace-item index counter-list-v3 new-count
  ]
  ;print count-group-list-v3
  ;print counter-list-v3
end

to get-count-with-double-bound-s-with-two-r
  set count-group-list-v4 []
  set counter-list-v4 []
  let unique-products remove-duplicates [product-id] of syks with [ site1-bound != nobody and  site2-bound != nobody]
  foreach unique-products
  [ pid ->
...
to get-count-of-groups
  set count-group-list []
  set counter-list []
  let unique-products remove-duplicates [product-id] of receptors with [ count receptors with [ product-id = [product-id] of myself ] > 1  and
  count syks with [ product-id = [product-id] of myself and site1-bound != nobody and  site2-bound != nobody] > 0]
  foreach unique-products
  [ pid ->
    let s-count count syks with [ product-id = pid and site1-bound != nobody and  site2-bound != nobody]
    let r-count count receptors with [ product-id = pid ]
    let combo (word r-count "R"  s-count "S")
    if not member? combo count-group-list
    [ set count-group-list lput combo count-group-list
      set counter-list lput 0 counter-list
    ]
    let index position combo count-group-list
    let new-count ( item index counter-list + 1)
    set counter-list replace-item index counter-list new-count
  ]
  ;print count-group-list
  ;print counter-list

end

to get-count-of-groups-v2
  set count-group-list-v2 []
  set counter-list-v2 []
  let unique-products remove-duplicates [product-id] of receptors with [ count receptors with [ product-id = [product-id] of myself ] > 1  and
  count syks with [ product-id = [product-id] of myself] > 0]
  foreach unique-products
  [ pid ->
    let s3-count count syks with [ product-id = pid and kinectically-active? and site1-bound != nobody and  site2-bound != nobody]
    let s2-count count syks with [ product-id = pid and (not kinectically-active?) and site1-bound != nobody and  site2-bound != nobody]
    let s1-count count syks with [ product-id = pid and (not kinectically-active?) and site1-bound != nobody or  site2-bound != nobody]
    let r-count count receptors with [ product-id = pid ]
    let combo (word r-count "R"  s3-count "S3" s2-count "S2" s1-count "S1")
    if not member? combo count-group-list-v2
    [ set count-group-list-v2 lput combo count-group-list-v2
      set counter-list-v2 lput 0 counter-list-v2
    ]
    let index position combo count-group-list-v2
    let new-count ( item index counter-list-v2 + 1)
    set counter-list-v2 replace-item index counter-list-v2 new-count
  ]

  ;print count-group-list-v2
  ;print counter-list-v2
end
...
to-report receptors-in-cluster
  report count receptors with [ phosphorylated? and cluster-id != nobody  ] / r-absolute * 100
end
...
to-report double-syks-in-cluster
  report count syks with [ site1-bound != nobody and site2-bound != nobody and  cluster-id != nobody] / s-absolute * 100
end
...
to-report total-double-bound-syks
  report (count syks with [site1-bound != nobody and site2-bound != nobody]) / s-absolute * 100
end

to-report total-single-bound-syks
  report ((count syks with [site1-bound != nobody and site2-bound = nobody]) + (count syks with [site1-bound = nobody and site2-bound != nobody])) / s-absolute * 100
end

to-report total-none-bound-syks
  report (count syks with [site1-bound = nobody and site2-bound = nobody]) / s-absolute * 100
end
...
to-report count-rrs-molecules
  report (length remove-duplicates [product-id] of syks with [ site1-bound != nobody and site2-bound != nobody and count syks with [ product-id = [product-id] of myself ] = 1 and count receptors with [ product-id = [product-id] of myself ] = 2 ]) / S * 100
end

to-report count-rs-in-rrs-molecules
  report (length remove-duplicates [product-id] of syks with [ site1-bound != nobody and site2-bound != nobody and count syks with [ product-id = [product-id] of myself ] = 1 and count receptors with [ product-id = [product-id] of myself ] = 2 ]) / R * 100 * 2
end

to-report count-s2
  report (count syks with [ site1-bound != nobody and site2-bound != nobody ] ) / S * 100
end

to-report count-s1
  report count syks with [ (site1-bound != nobody and site2-bound = nobody)  or (site1-bound = nobody and site2-bound != nobody)] / S * 100
end
...
to-report double-syks-in
  report double-syks-in-cluster / S * 100
end

to-report double-syks-out
  report double-syks-out-cluster / S * 100
end
...
to-report tot-phos-receptors
  ...
  report count receptors with [ phosphorylated? ] / R * 100
end

to-report r-dim
  report count receptors with [ not empty? receptor-bound] / R * 100
end
...
to-report r-dis
  report count receptors with [ empty? receptor-bound and protein-bound = nobody and ligand-bound = nobody] / R * 100
end

to count-molecule-with-syk
  set molecule-with-syks-list []
  set counter-molecule-with-syk-list []
  if any? syks with [ site1-bound != nobody and site2-bound != nobody ]
  [
    let molecule-with-max-syk [product-id] of syks with [site1-bound != nobody and site2-bound != nobody] with-max [ count syks with [ product-id = [product-id] of myself ]]
    if not empty? molecule-with-max-syk
    [
      let pid one-of molecule-with-max-syk
      let max-num count syks with [ product-id = pid ]
      foreach (range max-num)
      [ counter ->
        set molecule-with-syks-list lput (word (counter + 1) "S") molecule-with-syks-list
        let molecule-count length remove-duplicates [product-id] of syks with [ count syks with [ product-id = [product-id] of myself ] = (counter + 1) ]
        set counter-molecule-with-syk-list lput molecule-count counter-molecule-with-syk-list
      ]
    ]
  ]
...
to-report exact-rrs-count
  let exact-rrs 0
  if any? syks with [ site1-bound != nobody and site2-bound != nobody and count syks with [product-id = [product-id] of myself] = 1]
  [
    let uniq-pid remove-duplicates [product-id] of syks with [ site1-bound != nobody and site2-bound != nobody and count syks with [product-id = [product-id] of myself] = 1 ]
    foreach uniq-pid
    [ upid ->
      if count receptors with [ product-id = upid ] = 2 and count ligands with [ product-id = upid ] = 0
      [
        set exact-rrs (exact-rrs + 1 )
      ]
    ]
  ]
  report exact-rrs
...
to count-molecule-with-receptors

  set molecule-with-receptors-list []
  set counter-molecule-with-receptor-list []
  if any? receptors with [ count receptors with [ product-id = [product-id] of myself ] >= 2 ]
  [
    let molecule-with-max-receptor [product-id] of receptors with [count receptors with [product-id = [product-id] of myself] >= 2] with-max [ count receptors with [ product-id = [product-id] of myself ]]
    if not empty? molecule-with-max-receptor
    [
      let pid one-of molecule-with-max-receptor
      let max-num count receptors with [ product-id = pid ]
      foreach (range 2 (max-num + 1))
      [ counter ->
        let molecule-count length remove-duplicates [product-id] of receptors with [ count receptors with [ product-id = [product-id] of myself ] = counter ]
        if molecule-count > 0
        [
          set molecule-with-receptors-list lput (word counter "R") molecule-with-receptors-list
          set counter-molecule-with-receptor-list lput molecule-count counter-molecule-with-receptor-list
        ]
      ]
    ]
  ]
...
to count-molecule-with-receptors-for-hist
  set counter-receptor-list []
  if any? receptors with [ count receptors with [ product-id = [product-id] of myself ] >= 2 ]
  [
    let uniq-pid remove-duplicates [product-id] of receptors with [ count receptors with [ product-id = [product-id] of myself ] >= 2 ]
    foreach uniq-pid
    [ upid ->
      let count-of-receptor count receptors with [ product-id = upid ]
      set counter-receptor-list lput count-of-receptor counter-receptor-list
    ]
  ]
...
to-report count-of-receptors [req-count]
  report item (req-count - 2) counter-molecule-with-receptor-list
end

to count-molecule-with-receptors-dict
  set tot-r-dict table:make
  if any? receptors with [ count receptors with [ product-id = [product-id] of myself ] >= 2 ]
  [
    let molecule-with-max-receptor [product-id] of receptors with [count receptors with [product-id = [product-id] of myself] >= 2] with-max [ count receptors with [ product-id = [product-id] of myself ]]
    ...
    [
      ...
      foreach (range 2 (max-num + 1))
      [ counter ->
        ; let x (word counter "R")
        let molecule-count length remove-duplicates [product-id] of receptors with [ count receptors with [ product-id = [product-id] of myself ] = counter ]
        if molecule-count > 0
        [ table:put tot-r-dict (word counter "R") molecule-count ]
        ;table:put tot-r-dict molecule-count
      ]
    ]
  ]
...
to  write-cluster-characteristics
  let file-name "cluster-characteristics-extra.csv"
  file-open file-name
  file-print "RunNo., Ticks, ClusterID, MeanClusterCoefficient, MeanDegree, MinDegree, MaxDegree, CountTurtleWithMinDegree, CountTurtleWithMaxDegree, MeanBetween, MinBetween, MaxBetween, CountTurtleWithMinBetween, CountTurtleWithMaxBetween, Modularity, MEanPageRank "
  let uniq-cid []
  if any? turtles with [cluster-id != nobody]
  [
    set uniq-cid remove-duplicates [cluster-id] of turtles with [cluster-id != nobody]
  ]
  foreach uniq-cid
  [ cid ->
    let line (word behaviorspace-run-number "," ticks "," cid "," mean [nw:clustering-coefficient ] of turtles with [ cluster-id = cid ] "," mean [length [self] of link-neighbors] of turtles with [ cluster-id = cid ])
    set line ( word line "," min [length [self] of link-neighbors] of turtles with [ cluster-id = cid ] "," max [length [self] of link-neighbors] of turtles with [ cluster-id = cid ])
    set line ( word line "," count turtles with [ cluster-id = cid] with-min [[length [self] of link-neighbors] of turtles with [ cluster-id = cid ]]"," count turtles with [ cluster-id = cid] with-max[[length [self] of link-neighbors] of turtles with [ cluster-id = cid ]])
    set line ( word line mean [nw:betweenness-centrality ] of turtles with [ cluster-id = cid ] "," min [nw:betweenness-centrality ] of turtles with [ cluster-id = cid ] "," max [nw:betweenness-centrality ] of turtles with [ cluster-id = cid ] )
    set line ( word line "," count turtles with [ cluster-id = cid] with-min [[nw:betweenness-centrality ] of turtles with [ cluster-id = cid ]]"," count turtles with [ cluster-id = cid] with-max [[nw:betweenness-centrality ] of turtles with [ cluster-id = cid ]] )
    set line ( word line "," nw:modularity (list (turtles with [ cluster-id = cid])) ","   mean [nw:page-rank ] of turtles with [ cluster-id = cid ])
    file-print line
  ]
  file-close
  ask links [ set hidden? false ]
  nw:save-matrix (word "./matrixfiles/matrix-" ticks ".txt")
  nw:save-graphml (word "./graphMLfiles/tick-" ticks ".graphml")
...
"" ""
PENS
"R-dis" 1.0 0 -2674135 true "" "plot r-dis"
"R-dim" 1.0 0 -13840069 true "" "plot r-dim"
"R-phos" 1.0 0 -7500403 true "" "plot r-phos"
...
938
R-phos
count receptors with [ phosphorylated? ] / r-absolute * 100
2
1
16
...
1051
unphos-R %
count receptors with [ not phosphorylated? ] / r-absolute * 100
2
1
11
...
1051
blue-R
count receptors with [ color = blue ] /  r-absolute * 100
2
1
11
...
"" ""
PENS
"C-tot" 1.0 0 -16777216 true "" "plot item 0 cluster-number ;cluster-count "
"C-size" 1.0 0 -2674135 true "" "plot item 1 cluster-number ;mean-size"
...
659
582
Slow down rate of dephosphorylation within clusters:
11
0.0
1
...
"" ""
PENS
"S2+S3" 1.0 0 -16777216 true "" "if S > 0 [ plot (total-double-bound-syks) / s-absolute * 100 ]"
"count-S2" 1.0 0 -7500403 true "" "if S > 0 [ plot count-s2  ]"
"count-S1" 1.0 0 -13840069 true "" "if S > 0 [ plot count-s1  ]"
"count-S0" 1.0 0 -4079321 true "" "if S > 0 [ plot count-s0  ]"
...
"" ""
PENS
"R" 1.0 0 -16777216 true "" "plot count-rs-in-rrs-molecules"
"S" 1.0 0 -2674135 true "" "plot count-rrs-molecules"
...
626
NIL
counter-molecule-with-receptor-list
17
1
11
...
true
15
Rectangle -16777216 true false 60 140 238 150
Rectangle -7500403 false false 60 150 255 150
Rectangle -1 true true 5 109 75 184
Rectangle -16777216 false false 5 109 77 184
Rectangle -1 true true 220 108 290 183
Rectangle -16777216 false false 220 108 292 183
...
<experiments>
  ...
  <experiment name="experiment" repetitions="50" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="initial-R1-presence">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-7">
      <value value="86"/>
  ...
  <experiment name="vary off rate theme 2 at 20% R" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
    <metric>Count receptors with [ not empty? receptor-bound] / R * 100</metric>
  ...
  <experiment name="changeR1000ticks" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>Count receptors with [ not empty? receptor-bound] / R * 100</metric>
  ...
  <experiment name="experimentZ" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="100"/>
    <metric>count turtles</metric>
    ...
    <enumeratedValueSet variable="r-size">
      <value value="2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-6">
      <value value="0"/>
    ...
    <enumeratedValueSet variable="world-dimension">
      <value value="40"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-7">
      <value value="50"/>
    ...
    <enumeratedValueSet variable="my-patch-size">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-3">
      <value value="0"/>
    ...
    <enumeratedValueSet variable="world-dimension2">
      <value value="-40"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="varymovement" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>Count receptors with [ not empty? receptor-bound] / R * 100</metric>
  ...
  <experiment name="varyofftheme2" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>count receptors with [ phosphorylated? ] / R * 100</metric>
    <enumeratedValueSet variable="step-size">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="csv-output?">
  ...
  <experiment name="varyofftheme1withdimerisation" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>count receptors with [ phosphorylated? ] / R * 100</metric>
  ...
  <experiment name="s2reversible" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>count receptors with [ phosphorylated? ] / R * 100</metric>
  ...
  <experiment name="s2irrreversible2" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>count receptors with [ phosphorylated? ] / R * 100</metric>
  ...
  <experiment name="test_varymovement_25runs" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>Count receptors with [ not empty? receptor-bound] / R * 100</metric>
  ...
  <experiment name="fig8" repetitions="10" runMetricsEveryStep="true">
    ...
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>count receptors with [ phosphorylated? ] / r-absolute * 100</metric>
    <metric>Count receptors with [ not empty? receptor-bound] / r-absolute * 100</metric>
    <metric>cluster-count</metric>
    <metric>mean-size</metric>
    <enumeratedValueSet variable="step-size">
      <value value="0"/>
    ...
    <enumeratedValueSet variable="dense-area">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="check-steady-state?">
      <value value="false"/>
    ...
    <enumeratedValueSet variable="dense-region?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-3">
      <value value="1"/>
    ...
    <enumeratedValueSet variable="dense-size">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="theme-7">
      <value value="false"/>
  ...
  <experiment name="experimentFig815DEC" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <metric>Count receptors with [ not empty? receptor-bound] / r-absolute * 100</metric>
    <metric>count receptors with [ phosphorylated? ] / r-absolute * 100</metric>
    <metric>dimer-count / r-absolute * 100</metric>
    <metric>trimer-count * 3 / r-absolute * 100</metric>
    <metric>tetramer-count * 4 / r-absolute * 100</metric>
    <enumeratedValueSet variable="step-size">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Rate-of-dephosphorylation">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="csv-output?">
      <value value="false"/>
  ...
  <experiment name="Fig9-withoutS" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>cluster-count</metric>
  ...
  <experiment name="Fig9-withS" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>cluster-count</metric>
  ...
  <experiment name="fif6B" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
    <metric>Count receptors with [ not empty? receptor-bound] / r-absolute * 100</metric>
  ...
  <experiment name="Fig7" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <metric>mean-size</metric>
    <metric>r-dim</metric>
    <metric>r-phos</metric>
    <metric>dimer-count / r-absolute * 100</metric>
    <metric>dimer-count / r-absolute * 100</metric>
    <metric>sdimer-count / r-absolute * 100</metric>
    <metric>ssdimer-count / r-absolute * 100</metric>
    <metric>tdimer-count / r-absolute * 100</metric>
    <metric>trimer-count / r-absolute * 100</metric>
    <metric>strimer-count / r-absolute * 100</metric>
    <metric>sstrimer-count / r-absolute * 100</metric>
    <metric>ttrimer-count / r-absolute * 100</metric>
    <metric>tetramer-count / r-absolute * 100</metric>
    <metric>stetramer-count / r-absolute * 100</metric>
    <metric>sstetramer-count / r-absolute * 100</metric>
    <metric>ttetramer-count / r-absolute * 100</metric>
    <metric>pentamer-count / r-absolute * 100</metric>
    <metric>spentamer-count / r-absolute * 100</metric>
    <metric>sspentamer-count / r-absolute * 100</metric>
    <metric>sspentamer-count / r-absolute * 100</metric>
    <metric>tpentamer-count / r-absolute * 100</metric>
    <metric>hexamer-count / r-absolute * 100</metric>
    <metric>shexamer-count / r-absolute * 100</metric>
    <metric>sshexamer-count / r-absolute * 100</metric>
    <metric>thexamer-count / r-absolute * 100</metric>
    <metric>plus-count / r-absolute * 100</metric>
    <metric>splus-count / r-absolute * 100</metric>
    <metric>ssplus-count / r-absolute * 100</metric>
    <metric>tplus-count / r-absolute * 100</metric>
    <metric>double-syks-in-cluster / s-absolute * 100</metric>
    <metric>double-syks-out-cluster / s-absolute * 100</metric>
    <metric>receptors-in-cluster / r-absolute * 100</metric>
    <metric>receptors-out-cluster / r-absolute * 100</metric>
    <enumeratedValueSet variable="step-size">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Rate-of-dephosphorylation">
  ...
  <experiment name="Fig7 PI3K" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="10000"/>
    <metric>cluster-count</metric>
    ...
    <metric>r-dim</metric>
    <metric>r-phos</metric>
    <metric>disengaged-receptors</metric>
    <metric>total-double-bound-syks</metric>
    <metric>total-single-bound-syks</metric>
    <metric>total-none-bound-syks</metric>
    <metric>double-syks-in-cluster / s-absolute * 100</metric>
    <metric>double-syks-out-cluster / s-absolute * 100</metric>
    <metric>receptors-in-cluster / r-absolute * 100</metric>
    <metric>receptors-out-cluster / r-absolute * 100</metric>
    <metric>t-dimer-count t-trimer-count t-tetramer-count t-pentamer-count t-hexamer-count t-septamer-count t-octamer-count t-enneamer-count t-decamer-count t-hendecamer-count t-dodecamer-count t-triskaidecamer-count t-tetrakaidecamer-count t-pentakaidecamer-count t-hexakaidecamer-count t-heptakaidecamer-count t-octakaidecamer-count t-enneakaidecamer-count t-icosamer-count t-plus-count</metric>
    <metric>ts-dimer-count ts-trimer-count ts-tetramer-count ts-pentamer-count ts-hexamer-count ts-septamer-count ts-octamer-count ts-enneamer-count ts-decamer-count ts-hendecamer-count ts-dodecamer-count ts-triskaidecamer-count ts-tetrakaidecamer-count ts-pentakaidecamer-count ts-hexakaidecamer-count ts-heptakaidecamer-count ts-octakaidecamer-count ts-enneakaidecamer-count ts-icosamer-count ts-plus-count</metric>
    <metric>dimer-count trimer-count tetramer-count pentamer-count hexamer-count septamer-count octamer-count enneamer-count decamer-count hendecamer-count dodecamer-count triskaidecamer-count tetrakaidecamer-count pentakaidecamer-count hexakaidecamer-count heptakaidecamer-count octakaidecamer-count enneakaidecamer-count icosamer-count plus-count</metric>
    <metric>s-dimer-count s-trimer-count s-tetramer-count s-pentamer-count s-hexamer-count s-septamer-count s-octamer-count s-enneamer-count s-decamer-count s-hendecamer-count s-dodecamer-count s-triskaidecamer-count s-tetrakaidecamer-count s-pentakaidecamer-count s-hexakaidecamer-count s-heptakaidecamer-count s-octakaidecamer-count s-enneakaidecamer-count s-icosamer-count s-plus-count</metric>
    <metric>ss-dimer-count ss-trimer-count ss-tetramer-count ss-pentamer-count ss-hexamer-count ss-septamer-count ss-octamer-count ss-enneamer-count ss-decamer-count ss-hendecamer-count ss-dodecamer-count ss-triskaidecamer-count ss-tetrakaidecamer-count ss-pentakaidecamer-count ss-hexakaidecamer-count ss-heptakaidecamer-count ss-octakaidecamer-count ss-enneakaidecamer-count ss-icosamer-count ss-plus-count</metric>
    <metric>sss-dimer-count sss-trimer-count sss-tetramer-count sss-pentamer-count sss-hexamer-count sss-septamer-count sss-octamer-count sss-enneamer-count sss-decamer-count sss-hendecamer-count sss-dodecamer-count sss-triskaidecamer-count sss-tetrakaidecamer-count sss-pentakaidecamer-count sss-hexakaidecamer-count sss-heptakaidecamer-count sss-octakaidecamer-count sss-enneakaidecamer-count sss-icosamer-count sss-plus-count</metric>
    <metric>ssss-dimer-count ssss-trimer-count ssss-tetramer-count ssss-pentamer-count ssss-hexamer-count ssss-septamer-count ssss-octamer-count ssss-enneamer-count ssss-decamer-count ssss-hendecamer-count ssss-dodecamer-count ssss-triskaidecamer-count ssss-tetrakaidecamer-count ssss-pentakaidecamer-count ssss-hexakaidecamer-count ssss-heptakaidecamer-count ssss-octakaidecamer-count ssss-enneakaidecamer-count ssss-icosamer-count ssss-plus-count</metric>
    <metric>sssss-dimer-count sssss-trimer-count sssss-tetramer-count sssss-pentamer-count sssss-hexamer-count sssss-septamer-count sssss-octamer-count sssss-enneamer-count sssss-decamer-count sssss-hendecamer-count sssss-dodecamer-count sssss-triskaidecamer-count sssss-tetrakaidecamer-count sssss-pentakaidecamer-count sssss-hexakaidecamer-count sssss-heptakaidecamer-count sssss-octakaidecamer-count sssss-enneakaidecamer-count sssss-icosamer-count sssss-plus-count</metric>
    <metric>ssssss-dimer-count ssssss-trimer-count ssssss-tetramer-count ssssss-pentamer-count ssssss-hexamer-count ssssss-septamer-count ssssss-octamer-count ssssss-enneamer-count ssssss-decamer-count ssssss-hendecamer-count ssssss-dodecamer-count ssssss-triskaidecamer-count ssssss-tetrakaidecamer-count ssssss-pentakaidecamer-count ssssss-hexakaidecamer-count ssssss-heptakaidecamer-count ssssss-octakaidecamer-count ssssss-enneakaidecamer-count ssssss-icosamer-count ssssss-plus-count</metric>
    <metric>sssssss-dimer-count sssssss-trimer-count sssssss-tetramer-count sssssss-pentamer-count sssssss-hexamer-count sssssss-septamer-count sssssss-octamer-count sssssss-enneamer-count sssssss-decamer-count sssssss-hendecamer-count sssssss-dodecamer-count sssssss-triskaidecamer-count sssssss-tetrakaidecamer-count sssssss-pentakaidecamer-count sssssss-hexakaidecamer-count sssssss-heptakaidecamer-count sssssss-octakaidecamer-count sssssss-enneakaidecamer-count sssssss-icosamer-count sssssss-plus-count</metric>
    <metric>ssssssss-dimer-count ssssssss-trimer-count ssssssss-tetramer-count ssssssss-pentamer-count ssssssss-hexamer-count ssssssss-septamer-count ssssssss-octamer-count ssssssss-enneamer-count ssssssss-decamer-count ssssssss-hendecamer-count ssssssss-dodecamer-count ssssssss-triskaidecamer-count ssssssss-tetrakaidecamer-count ssssssss-pentakaidecamer-count ssssssss-hexakaidecamer-count ssssssss-heptakaidecamer-count ssssssss-octakaidecamer-count ssssssss-enneakaidecamer-count ssssssss-icosamer-count ssssssss-plus-count</metric>
    <metric>sssssssss-dimer-count sssssssss-trimer-count sssssssss-tetramer-count sssssssss-pentamer-count sssssssss-hexamer-count sssssssss-septamer-count sssssssss-octamer-count sssssssss-enneamer-count sssssssss-decamer-count sssssssss-hendecamer-count sssssssss-dodecamer-count sssssssss-triskaidecamer-count sssssssss-tetrakaidecamer-count sssssssss-pentakaidecamer-count sssssssss-hexakaidecamer-count sssssssss-heptakaidecamer-count sssssssss-octakaidecamer-count sssssssss-enneakaidecamer-count sssssssss-icosamer-count sssssssss-plus-count</metric>
    <metric>d-dimer-count d-trimer-count d-tetramer-count d-pentamer-count d-hexamer-count d-septamer-count d-octamer-count d-enneamer-count d-decamer-count d-hendecamer-count d-dodecamer-count d-triskaidecamer-count d-tetrakaidecamer-count d-pentakaidecamer-count d-hexakaidecamer-count d-heptakaidecamer-count d-octakaidecamer-count d-enneakaidecamer-count d-icosamer-count d-plus-count</metric>
    <metric>ds-dimer-count ds-trimer-count ds-tetramer-count ds-pentamer-count ds-hexamer-count ds-septamer-count ds-octamer-count ds-enneamer-count ds-decamer-count ds-hendecamer-count ds-dodecamer-count ds-triskaidecamer-count ds-tetrakaidecamer-count ds-pentakaidecamer-count ds-hexakaidecamer-count ds-heptakaidecamer-count ds-octakaidecamer-count ds-enneakaidecamer-count ds-icosamer-count ds-plus-count</metric>
    <metric>dss-dimer-count dss-trimer-count dss-tetramer-count dss-pentamer-count dss-hexamer-count dss-septamer-count dss-octamer-count dss-enneamer-count dss-decamer-count dss-hendecamer-count dss-dodecamer-count dss-triskaidecamer-count dss-tetrakaidecamer-count dss-pentakaidecamer-count dss-hexakaidecamer-count dss-heptakaidecamer-count dss-octakaidecamer-count dss-enneakaidecamer-count dss-icosamer-count dss-plus-count</metric>
    <metric>dsss-dimer-count dsss-trimer-count dsss-tetramer-count dsss-pentamer-count dsss-hexamer-count dsss-septamer-count dsss-octamer-count dsss-enneamer-count dsss-decamer-count dsss-hendecamer-count dsss-dodecamer-count dsss-triskaidecamer-count dsss-tetrakaidecamer-count dsss-pentakaidecamer-count dsss-hexakaidecamer-count dsss-heptakaidecamer-count dsss-octakaidecamer-count dsss-enneakaidecamer-count dsss-icosamer-count dsss-plus-count</metric>
    <metric>dssss-dimer-count dssss-trimer-count dssss-tetramer-count dssss-pentamer-count dssss-hexamer-count dssss-septamer-count dssss-octamer-count dssss-enneamer-count dssss-decamer-count dssss-hendecamer-count dssss-dodecamer-count dssss-triskaidecamer-count dssss-tetrakaidecamer-count dssss-pentakaidecamer-count dssss-hexakaidecamer-count dssss-heptakaidecamer-count dssss-octakaidecamer-count dssss-enneakaidecamer-count dssss-icosamer-count dssss-plus-count</metric>
    <metric>dsssss-dimer-count dsssss-trimer-count dsssss-tetramer-count dsssss-pentamer-count dsssss-hexamer-count dsssss-septamer-count dsssss-octamer-count dsssss-enneamer-count dsssss-decamer-count dsssss-hendecamer-count dsssss-dodecamer-count dsssss-triskaidecamer-count dsssss-tetrakaidecamer-count dsssss-pentakaidecamer-count dsssss-hexakaidecamer-count dsssss-heptakaidecamer-count dsssss-octakaidecamer-count dsssss-enneakaidecamer-count dsssss-icosamer-count dsssss-plus-count</metric>
    <metric>dssssss-dimer-count dssssss-trimer-count dssssss-tetramer-count dssssss-pentamer-count dssssss-hexamer-count dssssss-septamer-count dssssss-octamer-count dssssss-enneamer-count dssssss-decamer-count dssssss-hendecamer-count dssssss-dodecamer-count dssssss-triskaidecamer-count dssssss-tetrakaidecamer-count dssssss-pentakaidecamer-count dssssss-hexakaidecamer-count dssssss-heptakaidecamer-count dssssss-octakaidecamer-count dssssss-enneakaidecamer-count dssssss-icosamer-count dssssss-plus-count</metric>
    <metric>dsssssss-dimer-count dsssssss-trimer-count dsssssss-tetramer-count dsssssss-pentamer-count dsssssss-hexamer-count dsssssss-septamer-count dsssssss-octamer-count dsssssss-enneamer-count dsssssss-decamer-count dsssssss-hendecamer-count dsssssss-dodecamer-count dsssssss-triskaidecamer-count dsssssss-tetrakaidecamer-count dsssssss-pentakaidecamer-count dsssssss-hexakaidecamer-count dsssssss-heptakaidecamer-count dsssssss-octakaidecamer-count dsssssss-enneakaidecamer-count dsssssss-icosamer-count dsssssss-plus-count</metric>
    <metric>dssssssss-dimer-count dssssssss-trimer-count dssssssss-tetramer-count dssssssss-pentamer-count dssssssss-hexamer-count dssssssss-septamer-count dssssssss-octamer-count dssssssss-enneamer-count dssssssss-decamer-count dssssssss-hendecamer-count dssssssss-dodecamer-count dssssssss-triskaidecamer-count dssssssss-tetrakaidecamer-count dssssssss-pentakaidecamer-count dssssssss-hexakaidecamer-count dssssssss-heptakaidecamer-count dssssssss-octakaidecamer-count dssssssss-enneakaidecamer-count dssssssss-icosamer-count dssssssss-plus-count</metric>
    <metric>dsssssssss-dimer-count dsssssssss-trimer-count dsssssssss-tetramer-count dsssssssss-pentamer-count dsssssssss-hexamer-count dsssssssss-septamer-count dsssssssss-octamer-count dsssssssss-enneamer-count dsssssssss-decamer-count dsssssssss-hendecamer-count dsssssssss-dodecamer-count dsssssssss-triskaidecamer-count dsssssssss-tetrakaidecamer-count dsssssssss-pentakaidecamer-count dsssssssss-hexakaidecamer-count dsssssssss-heptakaidecamer-count dsssssssss-octakaidecamer-count dsssssssss-enneakaidecamer-count dsssssssss-icosamer-count dsssssssss-plus-count</metric>
    <metric>dd-dimer-count dd-trimer-count dd-tetramer-count dd-pentamer-count dd-hexamer-count dd-septamer-count dd-octamer-count dd-enneamer-count dd-decamer-count dd-hendecamer-count dd-dodecamer-count dd-triskaidecamer-count dd-tetrakaidecamer-count dd-pentakaidecamer-count dd-hexakaidecamer-count dd-heptakaidecamer-count dd-octakaidecamer-count dd-enneakaidecamer-count dd-icosamer-count dd-plus-count</metric>
    <enumeratedValueSet variable="step-size">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Rate-of-dephosphorylation">
  ...
  <experiment name="Fig7 PI3K TEST count-group-list" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>cluster-count</metric>
    ...
    <metric>receptors-in-cluster / r-absolute * 100</metric>
    <metric>receptors-out-cluster / r-absolute * 100</metric>
    <metric>count-group-list</metric>
    <enumeratedValueSet variable="step-size">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Rate-of-dephosphorylation">
  ...
  <experiment name="TEST count-group-list" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="Fig7 PI3K 26JAN2021" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="10000"/>
    <metric>cluster-count</metric>
    ...
    <metric>r-phos</metric>
    <metric>r-dis</metric>
    <metric>double-syks-in-cluster</metric>
    <metric>double-syks-out-cluster</metric>
    <metric>receptors-in-cluster</metric>
    <metric>receptors-out-cluster</metric>
    <metric>count-rrs-molecules</metric>
    <metric>count-s0</metric>
    <metric>couns-s1</metric>
    <metric>count-s2</metric>
    <metric>molecule-with-syks-list</metric>
    <metric>counter-molecule-with-syk-list</metric>
    <enumeratedValueSet variable="step-size">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Rate-of-dephosphorylation">
  ...
  <experiment name="Fig 7 PI3K 02FEB2021" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="10000"/>
    <metric>cluster-count</metric>
  ...
  <experiment name="Fig 7 PI3K 02FEB2021 more S values" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>r-phos</metric>
    <metric>r-dim</metric>
    <metric>r-dis</metric>
    <metric>exact-rrs-count</metric>
    <metric>count-s0</metric>
    <metric>count-s1</metric>
    <metric>count-s2</metric>
    <metric>molecule-with-syks-list</metric>
    <metric>counter-molecule-with-syk-list</metric>
    <metric>counter-receptor-list</metric>
    <metric>molecule-with-receptors-list</metric>
    <metric>counter-molecule-with-receptor-list</metric>
    <metric>2 * count-of-receptors 2</metric>
    <metric>3 * count-of-receptors 3</metric>
    <metric>4 * count-of-receptors 4</metric>
    <metric>5 * count-of-receptors 5</metric>
    <enumeratedValueSet variable="step-size">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Rate-of-dephosphorylation">
  ...
  <experiment name="Fig 8 PI3K 02FEB2021 more S values" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 02FEB2021 change on rate theme 9" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>r-phos</metric>
    ...
    <metric>molecule-with-receptors-list</metric>
    <metric>counter-molecule-with-receptor-list</metric>
    <metric>count-of-receptors 2</metric>
    <metric>count-of-receptors 3</metric>
    <metric>count-of-receptors 4</metric>
    <metric>count-of-receptors 5</metric>
    <enumeratedValueSet variable="step-size">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Rate-of-dephosphorylation">
  ...
  <experiment name="Fig 8 PI3K 08MAR2021 change R" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 02FEB2021 change on rate theme 4" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 02FEB2021 change on rate theme 2 S 50" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
    ...
    <metric>exact-rrs-count</metric>
    <metric>counter-receptor-list</metric>
    <metric>item 1 cluster-number</metric>
    <enumeratedValueSet variable="step-size">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Rate-of-dephosphorylation">
  ...
  <experiment name="Fig 7 PI3K 08MAR2021 change R" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K 02FEB2021 change on rate theme 2 S 25 LL 25" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K 08MAR2021 b change R" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K 08MAR2021 b change theme 5" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 02FEB2021 change on rate theme 4 S 50" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K 08MAR2021 b change theme 9" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 02FEB2021 change on rate theme 2" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 6 PI3K 04JUNE2021 change R" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="SYK vary k10 29APR2021" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="SYK FIG 8 vary k10 29APR2021" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 04JUNE2021 change off rate theme 2" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 08JUNE2021 change on rate theme 2 S 50" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 04JUNE2021 change on rate theme 1 S 50" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K15JUNE2021 change on rate theme 1 at 0.2 1 5 25 S 100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 08JUNE2021 change off rate theme 2 S 50" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 6 PI3K 09JUNE2021 change R with turtle speed = 2" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 6 PI3K 09JUNE2021 change R with turtle speed = 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="300"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 10JUNE2021 change on rate theme 2 without S" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 09JUNE2021 change off rate theme 2 without S" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 6 PI3K 14JUNE2021 change R speed" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 6 PI3K 04JUNE2021 change R testing for 10% dimerisation" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K15JUNE2021 change on rate theme 1 at 0.2 1 5 25 S 200 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K16JUNE2021 change S higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K 05JULY2021 change on rate k2 higher rate for themes 9 &amp; 4 at S200" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K16JUNE2021 change on rate k4 higher rate for themes 9 &amp; 4 at S200" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change on rate theme 1 at 0.2 1 5 25 S 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K 05JULY2021 change on rate k2 higher rate for themes 9 &amp; 4 at S100 LL100" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change R at 25 50 100 400 S 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change S at 25 50 100 200 R 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change LL at 25 50 100 200 R 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K13JULY2021 change L at 50 100 200 400 R 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Suppl. Fig 7 SYK themes 3&amp;10 on 30AUG2021 change theme 3 on rate 10 20 40 60 at  S 200 LL0 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Suppl. Fig 7 SYK themes 3&amp;10 on 30AUG2021 change theme 10 on rate 0.2 1 5 25 at  S 200 LL0 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Suppl. Fig 7 SYK themes 3&amp;10 on 30AUG2021 change theme 3 on rate 10 20 40 60 at  S 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Suppl. Fig 7 SYK themes 3&amp;10 on 30AUG2021 change theme 10 on rate 0.2 1 5 25 at  S 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change on rate theme 8 at 5 10 20 40 R 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change R at 25 50 100 400 S 200 LL0 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change on rate theme 9 at 10 20 40 60 S 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change on rate theme 4 at 10 20 40 60 S 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change S at 50 100 200 400 R 100 LL0 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change on rate theme 9 at 10 20 40 60 S 200 LL0 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change on rate theme 4 at 10 20 40 60 S 200 LL0 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K03SEP2021 change R at 25 50 100 400 S 100 LL100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K03SEP2021 change LL theme 2 off at LL 25 50 100 200 R 100 S0 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K03SEP2021 change LL theme 2 off at LL 25 50 100 200 R 100 S100 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K03SEP2021 change LL theme 2 ON at LL 25 50 100 200 R 100 S0 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K03SEP2021 change S theme 2 off at S 50 100 200 400 R 100 LL0 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K03SEP2021 change S theme 2 off at S 25 50 100 200 R 100 LL0 higher rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change off rate theme 9 at 10 20 40 60 S 200 LL0 higher rate for themes 9 &amp; 4, higher r-phos level (theme 1 on rate 11)" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change off rate theme 4 at 10 20 40 60 S 200 LL0 higher rate for themes 9 &amp; 4, higher r-phos level (theme 1 on rate 11)" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change S at 25 50 100 200 R 100 LL100 higher rate for themes 9 &amp; 4 at higher r-phos level" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change S at 25 50 100 200 R 100 LL0 higher rate for themes 9 &amp; 4 at higher r-phos level" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change off rate theme 9 at 10 20 40 60 S 100 LL0 higher rate for themes 9 &amp; 4, higher r-phos level (theme 1 on rate 11)" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change off rate theme 4 at 10 20 40 60 S 100 LL0 higher rate for themes 9 &amp; 4, higher r-phos level (theme 1 on rate 11)" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change S at 25 50 100 200 R 100 LL100 HIGHEST rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 7 PI3K15JUNE2021 change on rate theme 1 at 0.2 1 5 25 S 200 HIGHEST rate for themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
  ...
  <experiment name="Fig 8 PI3K05JULY2021 change S at 25 50 100 200 R 100 LL0 higher rate for themes 9 &amp; 4 at higher 50% r-phos level and HIGHEST on rates of themes 9 &amp; 4" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
    <metric>r-phos</metric>
